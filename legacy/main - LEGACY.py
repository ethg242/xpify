...
# Using Google App Engine and the webapp2 amd Jinja2 frameworks
#
###
		...
		if not gUser:
			...
			if userClasses:
				for classKey in userClasses:
					...
					classesHTML += '<li><a href="/class/challenges?class=%s">%s</a></li>\n' %(
							class_.classid, class_.classname)
				classesHTML += '<li id="current" class="join-class-plus"><a href="/class/search/"><div>+</div></a></li>'
			else:
				if user.usertype in ["Teacher", "Administrator"]:
					classesHTML = '''
<div id="no-classes">
...
<div id="no-classes">
	<span id="no-classes-banner">You have no classes!</span>
	<li id="current"><a href="/class/search/">
		Join one!
	</a></li>
</div>
'''
